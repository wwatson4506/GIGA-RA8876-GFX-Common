...
//    lcdDataWrite(XPCLK_INV << 7 | RA8876_DISPLAY_OFF << 6 | RA8876_OUTPUT_RGB);
    lcdDataWrite(XPCLK_INV << 7 | RA8876_DISPLAY_OFF << 6 | RA8876_OUTPUT_GRAY);
    ...
    _leftmarg = 0;
    _topmarg = 0;
    _rightmarg = (uint8_t)(_width / (_FNTwidth * _scaleX));
    _bottommarg = (uint8_t)(_height / (_FNTheight * _scaleY));
    _scrollXL = 0;
    _scrollXR = _width;
    _scrollYT = 0;
    _scrollYB = _height;
...
// Send a scaled string to the status line
//void RA8876_t3::slWrite(uint16_t x0,uint16_t fgColor,uint16_t bgColor, const char *text) {
...
// Clear current Screen within margins. Home Cursor. Set default forground color and set background color (bgc).
void RA8876_common::clearWindow(uint16_t bgc)
{
	drawSquareFill(_scrollXL, _scrollYT, _scrollXR, _scrollYB-STATUS_LINE_HEIGHT, bgc);
	textColor(_TXTForeColor,bgc);
	setTextCursor(_scrollXL,_scrollYT);
}
...
void RA8876_common::update_xy(void) {
   if(_cursorY >= (_scrollYB-STATUS_LINE_HEIGHT)) {
...
void RA8876_common::buildTextScreen(void) {
    ...
    _leftmarg = (uint8_t)(_scrollXL / (_FNTwidth * _scaleX));
    _topmarg = (uint8_t)(_scrollYT / (_FNTheight * _scaleY));
    _rightmarg = (uint8_t)(_scrollXR / (_FNTwidth * _scaleX));
    _bottommarg = (uint8_t)(_scrollYB / (_FNTheight * _scaleY));
    if (!UDFont) {
...
void RA8876_common::saveTFTParams(tftSave_t *screenSave) {
    ...
    screenSave->leftmarg = _leftmarg;
    screenSave->topmarg = _topmarg;
    screenSave->rightmarg = _rightmarg;
    screenSave->bottommarg = _bottommarg;
...
void RA8876_common::restoreTFTParams(tftSave_t *screenSave) {
    ...
    _leftmarg = screenSave->leftmarg;
    _topmarg = screenSave->topmarg;
    _rightmarg = screenSave->rightmarg;
    _bottommarg = screenSave->bottommarg;
...
void RA8876_common::writeRectImpl(int16_t x, int16_t y, int16_t w, int16_t h, const uint16_t *pcolors) {
	...
	switch (_rotation) {
		...
		case 2:
			{
				uint16_t *rotated_buffer_alloc = (uint16_t*)SDRAM.malloc(w* 2 + 32);
				...
				SDRAM.free((void *)rotated_buffer_alloc);  // Note: GIGA R1 used SDRAM.free().
			}
			break;
		case 3:
			{
...
uint16_t *RA8876_common::rotateImageRect(int16_t w, int16_t h, const uint16_t *pcolors, int16_t rotation) {
    uint16_t *rotated_colors_alloc = (uint16_t *)SDRAM.malloc(w * h * 2 + 32);  // Note: GIGA R1 used SDRAM.free().
    int16_t x, y;
...
void RA8876_common::initButton(struct Gbuttons *buttons, uint16_t x, uint16_t y, uint8_t w, uint8_t h,
...
void RA8876_common::drawButton(struct Gbuttons *buttons, boolean inverted) {
...
boolean RA8876_common::buttonContains(struct Gbuttons *buttons, uint16_t x, uint16_t y) {
...
void RA8876_common::buttonPress(struct Gbuttons *buttons, boolean p) {
  buttons->laststate = buttons->currstate;
  buttons->currstate = p;
}

// Check the current state of a button: pressed/released
boolean RA8876_common::buttonIsPressed(struct Gbuttons *buttons) {
	return buttons->currstate;
}

// Check if button was just pressed
boolean RA8876_common::buttonJustPressed(struct Gbuttons *buttons) {
	return (buttons->currstate && !buttons->laststate);
 }

// Check if button was just released
boolean RA8876_common::buttonJustReleased(struct Gbuttons *buttons) {
	return (!buttons->currstate && buttons->laststate);
}
//==========================================================================================
